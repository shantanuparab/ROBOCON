void Tz1() {
  ...
  if (rotateccw) {
    if (!readLine) {
      rotateCcw(5021);
      if ((abs(encoder0Reading) > 5000 && abs(encoder0Reading) < 5042) || (abs(encoder1Reading) > 5000 && abs(encoder1Reading) < 5042) || (abs(encoder2Reading) > 5000 && abs(encoder2Reading) < 5042)) {
        StopDrives();
        rotateccw = true;
        readLine = true;
        resetEncoders();
